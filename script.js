...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (getResultsButton) {
        getResultsButton.onclick = async function() {
            ...
            if (loadingText) {
                loadingText.textContent = "検索結果を取得しています...これには1-2分かかることがあります";
            }
...
// 抄録要約リクエストを順次実行する関数
async function sequentialProcessing(items, processFn) {
    const results = [];
...
検索式のみを出力してください。説明は不要です。マークダウンやコードブロックは使用しないでください。`;
        ...
        const data = await fetchWithRetry(`${API_BASE_URL}/api/openai`, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: "あなたは医学研究とシステマティックレビューの専門家です。PubMedの検索式作成に非常に長けています。検索式はマークダウンやコードブロックを使わず、プレーンテキストで提供してください。"
                    },
                    {
                        role: "user",
                        content: promptText
...
async function getSearchResults(searchExpression) {
    try {
        ...
        const searchData = await fetchWithRetry(`${API_BASE_URL}/api/pubmed/search?term=${encodedSearchTerm}&retmax=15`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        // 論文データを順次処理（並列処理からシーケンシャル処理に変更）
        const articles = await sequentialProcessing(pmids, async (pmid) => {
            try {
                            ...
                            const metaData = await fetchWithRetry(`${API_BASE_URL}/api/openai`, {
                                ...
                                body: JSON.stringify({
                                    ...
                                    messages: [
                                        {
                                            role: "system",
                                            content: "あなたは医学論文の抄録から情報を抽出する専門家です。抄録からタイトル、著者、出版年、ジャーナル名を見つけてください。マークダウンやコードブロックは使わずに、プレーンテキストのJSONで返してください。"
                                        },
                                        {
                                            role: "user",
...
${abstract}`
                            ...
                            try {
                                // JSONパース処理を改善
                                let content = metaData.choices[0].message.content;
    ...
    try {
        ...
        for (let i = 0; i < basicResults.length; i++) {
            ...
            try {
                ...
                // 一部処理済みの結果を表示
                if (i % 3 === 0 || i === basicResults.length - 1) {
...
async function fetchAbstract(pmid, retryCount = 0) {
    try {
        ...
        const url = `${API_BASE_URL}/api/pubmed/fetch?id=${pmid}&rettype=abstract`;
        console.log(`抄録取得リクエストURL: ${url}`);
        ...
        try {
            ...
            console.log(`PMID ${pmid} のfetchAbstract応答:`, response);
            ...
            if (response && response.abstract) {
                ...
                if (abstractText.length > 0) {
                    console.log(`抄録サンプル: ${abstractText.substring(0, 50)}...`);
                    return abstractText;
                } else {
                    console.log(`警告: 抄録テキストが空です`);
                }
            }
            
            // エラーが含まれているか確認
            if (response && response.error) {
                console.error(`PMID ${pmid} の抄録取得エラー:`, response.error);
            }
        } catch (backendError) {
            console.warn(`バックエンドAPI経由での抄録取得に失敗:`, backendError);
            // フォールバック処理に続く
        }
        
        // フォールバック: OpenAI APIを使って抄録を取得
        console.log(`フォールバック: OpenAI APIを使用してPMID ${pmid} の情報を取得`);
        
        // OpenAI APIを使用してPubMedから抄録を取得
        const openaiResponse = await fetchWithRetry(`${API_BASE_URL}/api/openai`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: "あなたはPubMedの論文情報を取得するアシスタントです。与えられたPubMed IDから論文の抄録を取得してください。"
                    },
                    {
                        role: "user",
                        content: `PubMed ID: ${pmid} の論文から抄録(Abstract)を取得し、そのままのテキストで返してください。
抄録が入手できない場合は「抄録は入手できませんでした」と返してください。
タイトルや著者など、他の情報は含めないでください。抄録のみを返してください。マークダウンやコードブロックは使わないでください。`
                    }
                ],
                temperature: 0.1
            })
        ...
        // OpenAI APIのレスポンスを処理
        const abstractText = openaiResponse.choices[0].message.content.trim();
        
        if (abstractText && abstractText !== "抄録は入手できませんでした" && abstractText.length > 20) {
            console.log(`PMID ${pmid} の抄録をOpenAI経由で取得成功 (長さ: ${abstractText.length}文字)`);
            return abstractText;
        }
...
抄録:
${abstract.substring(0, 1500)}` // 長すぎる抄録は切り詰める
                    }
                ],
                temperature: 0.3
            })
        ...
        if (firstLine.includes("Animal") || firstLine.includes("Basic")) return "Animal/Basic study";
...
マークダウンやコードブロックは使わないでください。プレーンテキストで回答してください。
...
async function generateAbstractSummary(abstract, retryCount = 0) {
    try {
        // 抄録が短すぎる場合はデフォルトメッセージを返す
        if (!abstract || abstract.length < 20) {
            return "抄録が提供されていません";
        }
        ...
        // トークン数を節約するために抄録を短くする
        const truncatedAbstract = abstract.length > 1500 ? 
        ...
        const data = await fetchWithRetry(`${API_BASE_URL}/api/openai`, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: "あなたは医学論文の抄録を短く要約する専門家です。日本語で簡潔に1文で要約してください。マークダウンやコードブロックは使わず、プレーンテキストで返してください。"
                    },
                    {
                        role: "user",
                        content: `以下の医学論文の抄録を日本語で1文に要約してください。マークダウンやコードブロックは使わず、プレーンテキストで返してください:\n\n${truncatedAbstract}`
                    }
                ],
                temperature: 0.3
            })
        ...
        // レート制限エラーの場合、遅延を増やして再試行
        if (error.message && error.message.includes('429') && retryCount < 2) {
            console.log(`レート制限エラー。再試行 ${retryCount + 1}/2...`);
            // 指数バックオフで待機時間を増加
            await new Promise(resolve => setTimeout(resolve, 3000 * Math.pow(2, retryCount)));
            return generateAbstractSummary(abstract, retryCount + 1);
        }
        
        // リトライ失敗またはその他のエラー
        if (abstract && abstract.length > 100) {
            // 抄録がある場合は最初の文を返す
            const firstSentence = abstract.split(/[.。]/).filter(s => s.trim().length > 0)[0];
            if (firstSentence && firstSentence.length > 20) {
                return `抄録の冒頭: ${firstSentence}...`;
            }
        }
...
function displaySearchResults(results) {
    ...
    results.forEach(function(article) {
        ...
        const abstractSummary = article.abstractSummary || '抄録情報なし';
        
        articleElement.innerHTML = `
            <div class="article-title">
                <a href="${article.pubmedUrl}" target="_blank">${title}</a>
            </div>
            ...
            <div class="article-summary">
                <strong>要約:</strong> ${abstractSummary}
            </div>
        `;
...
function displayEnhancedResults(results) {
    ...
    results.forEach(function(article) {
        ...
        const journal = article.journal || '';
        const year = article.year || '';
        const resultsSummary = article.resultsSummary || '結果情報なし';
        const conclusionSummary = article.conclusionSummary || '結論情報なし';
        ...
        articleElement.innerHTML = `
            ...
            <div class="article-summary">
                <strong>結果要約:</strong> ${resultsSummary}
            </div>
            <div class="article-summary">
                <strong>結論要約:</strong> ${conclusionSummary}
            </div>
        `;
